...
const MoviesCard = ({ film, savedMoviesToggle, filmsSaved }) => {
  ...
  return (
    <li className="card">
      ...
      <a className="card__image-content" href={pathname === '/saved-movies' ? film.trailerLink : film.trailerLink}
        target="_blank" rel="noreferrer">
        <img className="card__image" src={pathname === '/saved-movies' ? `${film.image}` :
          `https://api.nomoreparties.co${film.image.url}`} alt={film.nameRU}></img>
      </a>
    </li>
  );
};
